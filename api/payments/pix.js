...
export default async function handler(req,res){if(req.method!=='POST')return method(res,['POST']);if(!sameOrigin(req))return json(res,403,{error:'Origem da requisição não permitida.'});try{
  ...
  if(Number(order.totalCents)>PIX_MAX_CENTS)return json(res,422,{error:'O PIX desta conta PanteraPay exige que a cobrança fique abaixo de R$ 1.000,00. Remova itens até o total ficar dentro desse limite.'});
  ...
  else{providerBody=await createPix({amount:Number(order.totalCents),webhook:webhookUrl(req)});const pix=publicPix(providerBody);if(!pix.id)throw new Error('A PanteraPay não retornou o identificador da transação.');await updateOrder(admin,order.id,{providerReference:pix.id,status:orderStatusFromProvider(providerBody)});}
  const pix=publicPix(providerBody);return json(res,200,{orderId,status:orderStatusFromProvider(providerBody),pix});
}catch(e){console.error('[payments/pix]',e);const msg=e instanceof Error?e.message:'Não foi possível criar o PIX.';return json(res,502,{error:`A PanteraPay não conseguiu criar o PIX: ${msg}`});}}
