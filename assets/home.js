(async()=>{
  ...
  try{
    ...
    $('hero-name').textContent=hero.shortName;$('hero-description').textContent='Smartphones, áudio, streaming, wearables e acessórios com ofertas selecionadas.';$('hero-old').textContent=S.money(hero.originalPriceCents);$('hero-price').textContent=S.money(hero.promotionalPriceCents);$('hero-image').src=hero.variants[0].images[0].src;$('hero-image').alt=hero.variants[0].images[0].alt;$('hero-link').href=`/produto?id=${encodeURIComponent(hero.id)}`;
